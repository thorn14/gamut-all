...
import type { TokenRegistry, TokenInput } from '@gamut-all/core';

const tokensInput: TokenInput = {
  config: {
    wcagTarget: 'AA',
...
export function getTestRegistry(): TokenRegistry {
  if (!_registry) {
    const processed = processInput(tokensInput);
    _registry = buildRegistry(processed, wcag21);
  }
  return _registry;
}
